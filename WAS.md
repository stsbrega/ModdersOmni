# ModdersOmni — What Was Implemented
...
- **Auto-populated**: `DATABASE_URL_RAW` from Render's database reference (transformed at runtime to `postgresql+asyncpg://` format). `SECRET_KEY` auto-generated by Render.
- **Hardcoded in `render.yaml`** (auto-fill on deploy): `CORS_ORIGINS`, `FRONTEND_URL`, `LLM_PROVIDER` (`groq`), `GOOGLE_REDIRECT_URI`, `DISCORD_REDIRECT_URI`, `SMTP_HOST` (`smtp.gmail.com`), `SMTP_PORT` (`587`), frontend `API_URL`
- **Manual (`sync: false`)** — must be set in Render dashboard after first deploy: `GROQ_API_KEY`, `NEXUS_API_KEY`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `DISCORD_CLIENT_ID`, `DISCORD_CLIENT_SECRET`, `SMTP_USER`, `SMTP_PASSWORD`

#### Deployment Refinements (post-migration)
- Database plan set to **free** tier (sufficient for dev/early usage)
- Refresh token cookie set to `secure=True` for production HTTPS
